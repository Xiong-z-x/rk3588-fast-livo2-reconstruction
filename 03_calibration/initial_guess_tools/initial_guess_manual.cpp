...
public:
  ...
  bool image_window_exists() const {
    try {
      return cv::getWindowProperty("image", cv::WND_PROP_AUTOSIZE) >= 0.0;
    } catch (const cv::Exception&) {
      return false;
    }
  }

  void ensure_image_window() {
    if (!image_window_exists()) {
  ...
  void draw_image_cross(cv::Mat& canvas, const int x, const int y) {
  ...
  void update_image() {
    ...
    if (last_picked_image_pixel) {
      draw_image_cross(canvas, last_picked_image_pixel->x(), last_picked_image_pixel->y());
    }
  ...
  void on_mouse(int event, int x, int y, int flags) {
    ...
    last_picked_image_pixel = Eigen::Vector2i(x, y);
    draw_image_cross(canvas, x, y);
    cv::imshow("image", canvas);
...
private:
  ...
  std::optional<Eigen::Vector2i> last_picked_image_pixel;
